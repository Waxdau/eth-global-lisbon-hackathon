...
const aaPrivateKey = "0x8d011e5c49dfeebde11b597b77f1a363968a376bd97c5dc716c3b2c111d1ec56"
...
export default function Home() {
  ...
  useEffect(() => {
    const getAaProvider = async () => {
      ...
      const balance = await tempAaProvider.getBalance(await tempAaProvider.getSigner().getAddress());
      console.log('aabalance address: ', { balance, address: aaSigner.address })
  ...
  return (
    <main className="flex items-center justify-between p-24">
      <div>
        ...
        <p>Hardhat balance: {hhBalance ? ethers.utils.formatEther(hhBalance) : 0}</p>
        <button onClick={fundWallet}>Fund AA wallet</button>
      </div>
      <div>
        <div>Smart Account Balance: {balance ? ethers.utils.formatEther(balance) : 0}</div>
